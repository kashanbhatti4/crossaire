...
export default function Header() {
  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
      mobileMenuOpen 
        ? "bg-white border-b border-border-stroke" 
        : isScrolled 
          ? "bg-white border-b border-border-stroke shadow-[0_4px_20px_rgba(12,26,43,0.01)]" 
      ...
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <a href="#" className="flex items-center group logo-link" onClick={handleMobileLinkClick}>
          {/* White Logo (Shown when transparent & closed) */}
          <img 
            src="/media/images/logowhite.png" 
            alt="Crossaire Logo" 
            className={`h-10 w-auto object-contain transition-opacity duration-300 ${
              !isScrolled && !mobileMenuOpen ? "block" : "hidden"
            }`} 
          />
          {/* Dark Logo (Shown when scrolled or mobile menu open) */}
          <img 
        ...
        <nav className="hidden md:flex items-center gap-8 font-semibold text-sm">
          <a 
            href="#who-we-are" 
            className={`transition-colors nav-link-top ${
              isScrolled ? "text-primary-text hover:text-accent" : "text-white hover:text-accent"
            }`}
          >
            About Us
          </a>
          ...
          >
            <a 
              href="#services" 
              className={`flex items-center gap-1 transition-colors nav-link-top hover:text-accent ${
        ...
        <div className="hidden lg:flex items-center gap-6">
          <a 
            href="tel:8663992885" 
            className={`flex items-center gap-2 font-bold text-sm transition-colors nav-phone-link ${
              isScrolled ? "text-accent hover:text-accent-hover" : "text-white hover:text-accent"
            }`}
          >
            <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
              <path fill-rule="evenodd" d="M1.5 4.5a3 3 0 0 1 3-3h1.372c.86 0 1.61.586 1.819 1.42l1.105 4.423a1.875 1.875 0 0 1-.694 1.955l-1.293.97a1.521 1.521 0 0 0-.328 1.96 11.96 11.96 0 0 0 5.758 5.759 1.521 1.521 0 0 0 1.96-.328l.97-1.293a1.875 1.875 0 0 1 1.955-.694l4.423 1.105c.834.209 1.42.959 1.42 1.82V19.5a3 3 0 0 1-3 3h-2.25C8.552 22.5 1.5 15.448 1.5 6.75V4.5Z" clip-rule="evenodd" />
            </svg>
        ...
        >
          <svg 
            className={`w-6 h-6 fill-current ${mobileMenuOpen ? "hidden" : "block"} ${
              isScrolled ? "text-[#0C1A2B]" : "text-white"
            }`} 
            viewBox="0 0 24 24"
          >
          ...
          <svg 
            ...
            viewBox="0 0 24 24"
          >
            <path fill-rule="evenodd" d="M5.47 5.47a.75.75 0 0 1 1.06 0L12 10.94l5.47-5.47a.75.75 0 1 1 1.06 1.06L13.06 12l5.47 5.47a.75.75 0 1 1-1.06 1.06L12 13.06l-5.47 5.47a.75.75 0 1 1-1.06-1.06L12 13.06l-5.47 5.47a.75.75 0 1 1-1.06-1.06L10.94 12 5.47 6.53a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd" />
          </svg>
        </button>
      </div>
